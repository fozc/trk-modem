...
void http_handlers_reset(void)
{
    ...
    handler_state.is_authenticated = false;
    handler_state.current_query_string = NULL;
    HTTP_LOG_INF("[HTTP] Connection closed, session preserved\r\n");
}
...
void http_handlers_set_auth_from_token(const char *query_string)
{
    ...
    if ((bsp_get_tick() - handler_state.last_activity_tick) > HTTP_SESSION_TIMEOUT_MS) {
        ...
        HTTP_LOG_INF("[HTTP] Session expired\r\n");
        return;
    }
...
{
    HTTP_LOG_INF("[HTTP] POST /auth/login - Login attempt\r\n");
    ...
    HTTP_LOG_INF("[HTTP] Login attempt - Username: %s\r\n", username);
    ...
    HTTP_LOG_INF("[HTTP] Device IP octets - First: %d, Last: %d\r\n", ip_a, ip_d);
    ...
    {
        HTTP_LOG_ERR("[HTTP] Login password truncated - increase buffer!\r\n");
    }
    ...
    if (strcmp(username, USER_ROLE_ADMIN) == 0 && strcmp(password, expected_admin_pass) == 0) {
        ...
        HTTP_LOG_INF("[HTTP] Admin login successful\r\n");
        ...
        handler_state.username[sizeof(handler_state.username) - 1] = '\0';
        HTTP_LOG_INF("[HTTP] User login successful\r\n");
    } else {
        HTTP_LOG_ERR("[HTTP] Login failed - Invalid credentials\r\n");
    ...
    if (valid) {
        ...
        HTTP_LOG_INF("[HTTP] Session authenticated, token: %08lX\r\n", (unsigned long)new_token);
...
{
    HTTP_LOG_INF("[HTTP] POST /auth/logout - Logout request\r\n");
    ...
    HTTP_LOG_INF("[HTTP] Session cleared\r\n");
...
{
    HTTP_LOG_INF("[HTTP] GET /r?deviceConfig - Reading device config\r\n");
    const modem_config_t *config = get_device_config();
    if (!config) {
        HTTP_LOG_ERR("[HTTP] ERROR: Config not available\r\n");
    ...
    pos += xsnprintf(buf + pos, buf_size - pos, "}");
    HTTP_LOG_INF("[HTTP] JSON response size: %d bytes\r\n", pos);
    if (pos >= buf_size - 1) {
        HTTP_LOG_ERR("[HTTP] WARNING: Buffer nearly full! pos=%d, buf_size=%d\r\n", pos, buf_size);
    }
    http_send_json(buf, pos);
}
...
{
    HTTP_LOG_INF("[HTTP] POST /w?deviceConfig - Updating device config\r\n");
    
    if (!json_body) {
    	HTTP_LOG_ERR("[HTTP] ERROR: No JSON body\r\n");
        http_send_bad_request();
        return;
    }
    
    HTTP_LOG_INF("[HTTP] JSON body: %.100s...\r\n", json_body);
    ...
    if (!parse_device_config(json_body, &config)) {
    	HTTP_LOG_ERR("[HTTP] ERROR: JSON parse failed\r\n");
        http_send_error(400, "JSON parse error");
        return;
    }
    
    HTTP_LOG_INF("[HTTP] JSON parsed successfully\r\n");
    HTTP_LOG_INF("[HTTP] SimKartAPN: %s\r\n", config.apn.apn);
    ...
    HTTP_LOG_INF("[HTTP] Config saved to data_model\r\n");
...
{
    HTTP_LOG_INF("[HTTP] GET /r?boardStatus - Reading board status\r\n");
    
    const system_status_t *status = system_status_get();


    if (!status) {
        HTTP_LOG_ERR("[HTTP] ERROR: Board status not available\r\n");
    ...
    HTTP_LOG_INF("[HTTP] Board status JSON size: %d bytes\r\n", pos);
...
{
    HTTP_LOG_INF("[HTTP] GET /syslogs - Reading system logs\r\n");
    ...
    HTTP_LOG_INF("[HTTP] System Logs - offset=%lu, limit=%lu, total=%u, available=%u\r\n", 
...
#define SYSLOGS_HEADROOM_BYTES 256
    ...
    HTTP_LOG_INF("[HTTP] System Logs JSON size: %d bytes\r\n", pos);
...
{
    HTTP_LOG_INF("[HTTP] GET /r?iecConfig - Reading IEC config\r\n");
    
    const iec104_config_t *config = iec104_config_get();
    if (!config) {
    	HTTP_LOG_ERR("[HTTP] ERROR: IEC config not available\r\n");
    ...
    HTTP_LOG_INF("[HTTP] IEC config JSON size: %d bytes\r\n", pos);
...
{
    HTTP_LOG_INF("[HTTP] POST /w?iecConfig - Updating IEC config\r\n");
    ...
    HTTP_LOG_INF("[HTTP] JSON body length: %d bytes\r\n", body_len);
    HTTP_LOG_INF("[HTTP] JSON body (full): %s\r\n", json_body);
    
    // Create local config from current NVRAM state
    jiec_config_t config = {0};
    const iec104_config_t *nvram_config = iec104_config_get();
    if (!nvram_config) {
        HTTP_LOG_ERR("[HTTP] ERROR: Cannot read current config\r\n");
    ...
    HTTP_LOG_INF("[HTTP] JSON parsed successfully\r\n");
    HTTP_LOG_INF("[HTTP] ScadaIPAdresi: %s\r\n", config.scada_ip_address);
    HTTP_LOG_INF("[HTTP] ScadaPort: %u\r\n", config.scada_port);
    HTTP_LOG_INF("[HTTP] PeriyodikGonderimZamani: %u\r\n", config.periodical_send_interval);
    ...
    HTTP_LOG_INF("[HTTP] IEC config saved\r\n");
...
{
    HTTP_LOG_INF("[HTTP] GET /r?modbusConfigs - Reading Modbus config\r\n");
    
    const modbus_configs_t *config = modbus_config_get();
    if (!config) {
        HTTP_LOG_ERR("[HTTP] ERROR: Modbus config not available\r\n");
    ...
    HTTP_LOG_INF("[HTTP] Modbus config JSON size: %d bytes\r\n", pos);
...
{
    HTTP_LOG_INF("[HTTP] POST /w?modbusConfigs - Updating Modbus config\r\n");
    ...
    HTTP_LOG_INF("[HTTP] JSON parsed successfully\r\n");
    HTTP_LOG_INF("[HTTP] CihazID: %u\r\n", config.device_addr);
    HTTP_LOG_INF("[HTTP] BaudRate: %u\r\n", config.baud_rate);
    ...
    HTTP_LOG_INF("[HTTP] Modbus config %s\r\n", res == 0 ? "saved" : "save failed");
...
void handle_get_rf_discovery_json(void)
{
    ...
    HTTP_LOG_INF("[HTTP] GET /discovery/rf\r\n");
...
void handle_get_rf_config_json(void)
{
    ...
    HTTP_LOG_INF("[HTTP] GET /r?ayiriciRFConfig - Reading RF config\r\n");

    /* Govde uretimi tablo-tabanli rf_json modulundedir (kod hafizasi). */
    pos = rf_json_config_build(buf, buf_size);
    HTTP_LOG_INF("[HTTP] RF config JSON size: %d bytes\r\n", pos);
...
{
    HTTP_LOG_INF("[HTTP] POST /w?ayiriciRFConfig - Updating RF config\r\n");
    ...
    if (!rf_store_stage_begin()) {
    	HTTP_LOG_ERR("[HTTP] ERROR: RF staging busy\r\n");
    ...
    for (int i = 0; i < MAX_POWER_LINE_COUNT; i++) {
        if (config.in_use[i]) {
            HTTP_LOG_INF("[HTTP] Line %d: HatID=%u, ZoneID=%u, EUI-64=[%s, %s, %s]\r\n",
    ...
    if (rf_store_sync() != 0) {
        HTTP_LOG_ERR("[HTTP] ERROR: Failed to sync RF config to storage\r\n");
        http_send_error(500, "Failed to save configuration");
        return;
    }
    
    HTTP_LOG_INF("[HTTP] RF config saved\r\n");
...
static void send_rf_monitor_json(int line_filter)
{
    if (line_filter >= MAX_POWER_LINE_COUNT) {
        HTTP_LOG_ERR("[HTTP] ERROR: Invalid line index: %d\r\n", line_filter);
    ...
    for (int i = start_line; i < end_line; i++) {
        ...
        if (!monitor) {
            HTTP_LOG_ERR("[HTTP] ERROR: RF monitor data not available for line %d\r\n", i);
            continue;
        }
    ...
    pos += xsnprintf(buf + pos, buf_size - pos, "]}");
    HTTP_LOG_INF("[HTTP] RF monitor JSON size: %d bytes (lines: %d-%d)\r\n", pos, start_line + 1, end_line);
    if (pos >= buf_size - 1) {
        HTTP_LOG_ERR("[HTTP] WARNING: Buffer nearly full! pos=%d, buf_size=%d\r\n", pos, buf_size);
    }
    http_send_json(buf, pos);
...
{
    HTTP_LOG_INF("[HTTP] GET RF monitor for line %d\r\n", line_id);
...
{
    HTTP_LOG_INF("[HTTP] GET /r?ayiriciRFMonitor - Reading RF monitor data\r\n");
    ...
    if (handler_state.current_query_string) {
        ...
        if (http_get_query_param(handler_state.current_query_string, "line", line_value, sizeof(line_value))) {
            ...
            HTTP_LOG_INF("[HTTP] Line filter: %d\r\n", line_filter);
...
void fw_update_register_callbacks(const fw_update_callbacks_t *callbacks) {
    if (callbacks) {
        ...
        HTTP_LOG_INF("[FW] Firmware update callbacks registered\r\n");
    }
}
...
{
    HTTP_LOG_INF("[HTTP] GET /faults - Reading IEC104 fault records\r\n");
    ...
    if (feeder < 0 || feeder >= MAX_POWER_LINE_COUNT) {
        HTTP_LOG_INF("[HTTP] /faults: invalid feeder=%d\r\n", feeder);
        http_send_bad_request();
        return;
    }
    ...
    HTTP_LOG_INF("[HTTP] /faults JSON size: %d bytes (feeder %d)\r\n", pos, feeder);
    if (pos >= buf_size - 1) {
        HTTP_LOG_WRN("[HTTP] WARNING: /faults buffer nearly full! pos=%d, buf_size=%d\r\n", pos, buf_size);
    }
    http_send_json(buf, pos);
}

void handle_get_fw_version(void) {
    HTTP_LOG_INF("[FW] GET /r?fwVersion\r\n");
...
void handle_fw_start(const char *json_body) {
    HTTP_LOG_INF("[FW] POST /s (fw_start)\r\n");
    ...
    if (size == 0U) {
        HTTP_LOG_ERR("[FW] ERROR: Invalid size\r\n");
    ...
    {
        HTTP_LOG_INF_C(XCOLOR_GREEN, "[FW] RESUME: same file detected, continuing from %u/%u bytes\r\n",
                 fw_state.received_bytes, fw_state.total_size);
        char *buf = handler_state.tx_buffer;
    ...
    if (fw_state.in_progress) {
        HTTP_LOG_WRN("[FW] Different file - resetting previous upload (was %u/%u bytes)\r\n",
                 fw_state.received_bytes, fw_state.total_size);
    }

    HTTP_LOG_INF_C(XCOLOR_CYAN, "[FW] ========== FIRMWARE UPDATE START ==========\r\n");
    HTTP_LOG_INF("[FW] Total Size: %u bytes\r\n", size);
    HTTP_LOG_INF("[FW] Total Chunks: %u\r\n", chunks);
    HTTP_LOG_INF("[FW] File Hash: 0x%04X\r\n", hash);

    int result = fw_callbacks.fw_init(size);
    if (result != 0) {
        HTTP_LOG_ERR("[FW] ERROR: Flash init failed: %d\r\n", result);
    ...
    HTTP_LOG_INF_C(XCOLOR_GREEN, "[FW] Flash initialized, ready for chunks\r\n");
...
void handle_fw_chunk(uint32_t offset, const uint8_t *data, uint32_t size, 
    ...
    if (!fw_state.in_progress) {
        HTTP_LOG_ERR("[FW] ERROR: No update in progress\r\n");
    ...
    if (offset == 0 && size > 0) {
        ...
        HTTP_LOG_INF("[FW] First chunk size: %u bytes\r\n", size);
    }
    ...
    if (has_checksum) {
        if (calc_checksum != expected_checksum) {
            ...
            HTTP_LOG_ERR("[FW] CHECKSUM FAIL! Chunk #%u @offset=%u\r\n", chunk_num, offset);
            HTTP_LOG_ERR("[FW]   Expected: 0x%04X\r\n", expected_checksum);
            HTTP_LOG_ERR("[FW]   Received: 0x%04X\r\n", calc_checksum);
            HTTP_LOG_ERR("[FW]   Size: %u bytes, Total CS errors: %u\r\n", 
        ...
        HTTP_LOG_INF("[FW] Chunk #%u: offset=%u, size=%u, CS=0x%04X OK\r\n", 
                chunk_num, offset, size, calc_checksum);
    } else {
        /* No checksum provided - log warning */
        HTTP_LOG_WRN("[FW] Chunk #%u: offset=%u, size=%u (NO CHECKSUM)\r\n",
    ...
    if (offset < fw_state.received_bytes) {
        ...
        if (offset + size == fw_state.received_bytes) {
            HTTP_LOG_WRN("[FW] Retry chunk #%u accepted (total retries: %u)\r\n", 
        ...
        HTTP_LOG_WRN("[FW] Late retry? offset=%u < received=%u (chunk #%u)\r\n", 
    ...
    if (offset > fw_state.received_bytes) {
        ...
        HTTP_LOG_ERR("[FW] GAP DETECTED! offset=%u, expected=%u\r\n", 
                 offset, fw_state.received_bytes);
        HTTP_LOG_ERR("[FW]   Missing %u bytes (chunk #%u expected)\r\n",
    ...
    if (offset + size > fw_state.total_size) {
        HTTP_LOG_ERR("[FW] OVERFLOW! offset=%u + size=%u > total=%u\r\n", 
    ...
    if (result != 0) {
        HTTP_LOG_ERR("[FW] FLASH WRITE FAIL! offset=%u, error=%d\r\n", offset, result);
    ...
    if (progress / 10 != last_progress / 10) {
        HTTP_LOG_INF_C(XCOLOR_GREEN, "[FW] Progress: %u%% (%u/%u bytes, chunk %u/%u)\r\n", 
...
void handle_fw_finish(const char *json_body) {
    ...
    HTTP_LOG_INF("[FW] ---- Transfer Summary ----\r\n");
    HTTP_LOG_INF("[FW] Total Size:      %u bytes\r\n", fw_state.total_size);
    HTTP_LOG_INF("[FW] Received:        %u bytes\r\n", fw_state.received_bytes);
    HTTP_LOG_INF("[FW] Expected Chunks: %u\r\n", fw_state.total_chunks);
    HTTP_LOG_INF("[FW] Received Chunks: %u\r\n", fw_state.received_chunks);
    HTTP_LOG_INF("[FW] Checksum Errors: %u\r\n", fw_state.checksum_errors);
    HTTP_LOG_INF("[FW] Retry Count:     %u\r\n", fw_state.retry_count);
    
    if (fw_state.received_bytes != fw_state.total_size) {
        uint32_t missing = fw_state.total_size - fw_state.received_bytes;
        HTTP_LOG_ERR("[FW] ERROR: Incomplete transfer!\r\n");
        HTTP_LOG_ERR("[FW]   Missing: %u bytes (%.1f%%)\r\n", 
...
void handle_fw_reboot(void) {
    HTTP_LOG_INF_C(XCOLOR_CYAN, "[FW] ========== REBOOT REQUEST ==========\r\n");
    
    if (!fw_callbacks_registered || !fw_callbacks.fw_reboot) {
        HTTP_LOG_ERR("[FW] ERROR: Reboot callback not registered\r\n");
        http_send_json("{\"status\":\"error\",\"error\":\"not supported\"}", 41);
        return;
    }
...
void handle_fw_apply(void) {
    HTTP_LOG_INF_C(XCOLOR_CYAN, "[FW] ========== APPLY FW REQUEST ==========\r\n");

    if (!fw_state.in_progress && (0U == fw_state.total_size)) {
        HTTP_LOG_ERR("[FW] ERROR: No firmware downloaded\r\n");
        http_send_json("{\"status\":\"error\",\"error\":\"no firmware\"}", 43);
        return;
    }

    if (fw_state.received_bytes != fw_state.total_size) {
        HTTP_LOG_ERR("[FW] ERROR: Download incomplete\r\n");
        http_send_json("{\"status\":\"error\",\"error\":\"incomplete\"}", 42);
        return;
    }

    HTTP_LOG_WRN("[FW] Requesting bootloader update mode via IPC...\r\n");
    ...
    {
        HTTP_LOG_ERR("[FW] ERROR: IPC request failed (%d)\r\n", result);
        http_send_json("{\"status\":\"error\",\"error\":\"ipc failed\"}", 39);
        return;
    }

    HTTP_LOG_INF_C(XCOLOR_GREEN, "[FW] Update armed - reset in 10 s\r\n");
