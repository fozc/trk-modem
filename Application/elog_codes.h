...
typedef enum
{
	...
	ELOG_GSM_COLD_BOOT = 26,  /* info: attempt(1) - 1-based cold boot number */
	ELOG_GSM_WTD_LIVENESS = 27, /* info: silent_sec(2,BE) recovery_count(1) */
...
static inline const char* elog_code_to_string(elog_code_t code)
{
	switch(code) 
	{
		...
		case ELOG_GSM_COLD_BOOT: return "GSM_COLD_BOOT";
		case ELOG_GSM_WTD_LIVENESS: return "GSM_WTD_LIVENESS";
